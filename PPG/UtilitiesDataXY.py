...
#         xy_tr_val = [self.transformers.transform(self.dfs[i]) for i in train_idxes]
#         xy_ts = [self.transformers_ts.transform(self.dfs[i]) for i in [ts_sub]]

#         xy_tr, xy_val = self.split(xy_tr_val)
...
class JointTrValDataLoaderFactory():
    ...
    def make_loaders(self, ts_sub, train_ratio=0.8):
        ...
        xy_tr_val = self.make_set(xy_tr_val)
        xy_ts = self.make_set(xy_ts)
